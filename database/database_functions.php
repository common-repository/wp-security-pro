<?php
	...
	class Mo_wpmpDB
	{
		...
		function mo_plugin_activate(){
			global $wpdb;	
			if(!get_option('mo_mmp_dbversion')||get_option('mo_mmp_dbversion')< Mo_wp_Constants::DB_VERSION){
				update_option('mo_mmp_dbversion', Mo_wp_Constants::DB_VERSION );
				add_site_option('mo_mmp_switch_all', 1);
				$this->generate_tables();
			} else {
				$current_db_version = get_option('mo_mmp_dbversion');
				if($current_db_version < Mo_wp_Constants::DB_VERSION){
					update_option('mo_mmp_dbversion', Mo_wp_Constants::DB_VERSION );
	        ...
	        if(empty($row1) && empty($row1) && empty($row1)){
	            ...
	            if($result){
					...
					foreach ($report_ids as $key => $value) {
						$sql = $wpdb->prepare("SELECT report FROM $this->scanreportdetails WHERE report_id = %d",array($report_ids[$key]->id));
						$scan_detail = $wpdb->get_results($sql);
		...
		{
			global $wpdb;
			$sql = $wpdb->prepare("SELECT ip, type, time, input FROM ".$this->attackList." WHERE type = %s",array('LFI attack'));
		...
		{
			global $wpdb;
			$sql = $wpdb->prepare("SELECT ip, type, time, input FROM ".$this->attackList." WHERE type = %s",array('RCE attack'));
			return $wpdb->get_results($sql);
		...
		{
			global $wpdb;
			$sql = $wpdb->prepare("SELECT ip, type, time, input FROM ".$this->attackList." WHERE type = %s",array('XSS attack'));
			return $wpdb->get_results($sql);
		}
		...
		function get_count_of_attacks_blocked(){
			global $wpdb;
			return $wpdb->get_var( "SELECT COUNT(*) FROM ".$this->transactionTable." WHERE status = '".Mo_wp_Constants::FAILED."' OR status = '".Mo_wp_Constants::PAST_FAILED."'" );
		}

		function get_failed_transaction_count($ipAddress)
		{
			global $wpdb;
			return $wpdb->get_var( $wpdb->prepare("SELECT COUNT(*) FROM ".$this->transactionTable." WHERE ip_address = '%s' AND status = '".Mo_wp_Constants::FAILED."'", array($ipAddress)) );
		}
		...
		function delete_transaction($ipAddress)
		{
			global $wpdb;
			$wpdb->query( 
				$wpdb->prepare("DELETE FROM ".$this->transactionTable." WHERE ip_address = '%s' AND status='".Mo_wp_Constants::FAILED."'",array($ipAddress))
			);
			return;
		}
