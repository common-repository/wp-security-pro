    ...
    */
	...
	class WPSecurityPro{
		...
		function __construct()
		{
			...
			if (is_admin()) {
				add_filter('plugin_action_links', array($this, 'mo_wpns_add_plugin_action_links'), 10, 2);
			}
			...
			$this->includes();
			$notify = new Mo_wp_miniorange_security_notification;
		...
		function mo_wpns_add_plugin_action_links($links, $file) {
			if ($file == plugin_basename(dirname(__FILE__) . '/mo-wpns.php')) {
				$links[]= '<a href="admin.php?page=mo_mmp_dashboard" style="font-weight:bold" >Dashboard</a>';
				$links[]='<a href="admin.php?page=mo_mmp_malwarescan" style="font-weight:bold" >Malware scanner</a>';
				$links[]='<a href="admin.php?page=mo_mmp_waf" style="font-weight:bold" >WAF</a>';
        ...
        function feedback_request() {
            ...
            wp_enqueue_style( 'mo_wpns_admin_plugins_page_style', plugins_url( '/includes/css/style_settings.css', __FILE__ ) ,[],MO_WPNS_VERSION);
        ...
        function mo_mmp_redirect_page()
    	{
        	if(get_site_option('mo_mmp_plugin_redirect'))
        	{
            	...
            	wp_safe_redirect(admin_url().'admin.php?page=mo_mmp_dashboard');
                exit();
       	    }
    	}
        ...
        function mo_wpns_malware_notices(){
	    	...
	    	if(!get_option('donot_show_infected_file_notice') && (get_option('mo_mmp_infected_files') != 0 || get_option('mo_mmp_warning_files') !=0) && ($day_infected >= 1)){
	    		echo Mo_wp_Messages::showMessage('INFECTED_FILE');
	    	}else if(!get_option('donot_show_new_plugin_theme_notice') && ($flag_plugin || $flag_theme)){
	    		echo Mo_wp_Messages::showMessage('NEW_PLUGIN_THEME_CHECK');
	    	}else if(!get_option('donot_show_weekly_scan_notice') && ($days >= 7) && ($day_weekly >= 1)){
	    		echo Mo_wp_Messages::showMessage('WEEKLY_SCAN_CHECK');
	    	}
        }
		...
		function mo_wpns_widget_menu()
		{
			...
			add_submenu_page( $menu_slug	,'WP Security Pro'	,'Dashboard','administrator', 'mo_mmp_dashboard'	, array( $this, 'mo_wpns'));
			add_submenu_page( $menu_slug	,'WP Security Pro'	,'Malware Scan','administrator','mo_mmp_malwarescan'  	, array( $this, 'mo_wpns'));
			add_submenu_page( $menu_slug	,'WP Security Pro'	,'WAF'		   			,'administrator','mo_mmp_waf'				, array( $this, 'mo_wpns'));
			add_submenu_page( $menu_slug	,'WP Security Pro'	,'Login and Spam'		,'administrator','mo_mmp_login_and_spam'	, array( $this, 'mo_wpns'));
            add_submenu_page( $menu_slug	,'WP Security Pro'	,'Backup'				,'administrator','mo_mmp_backup'			, array( $this, 'mo_wpns'));
            add_submenu_page( $menu_slug	,'WP Security Pro'	,'Advanced Blocking'	,'administrator','mo_mmp_advancedblocking'	, array( $this, 'mo_wpns'));
            add_submenu_page( $menu_slug	,'WP Security Pro'	,'Notifications'		,'administrator','mo_mmp_notifications'	, array( $this, 'mo_wpns'));
            add_submenu_page( $menu_slug	,'WP Security Pro'	,'Reports'				,'administrator','mo_mmp_reports'			, array( $this, 'mo_wpns'));
            add_submenu_page( $menu_slug	,'WP Security Pro'	,'Account'				,'administrator','mo_mmp_account'			, array( $this, 'mo_wpns'));
            add_submenu_page( $menu_slug	,'WP Security Pro'	,'<strong style="color:orange">Upgrade</strong>'				,'administrator','mo_mmp_upgrade'			, array( $this, 'mo_wpns'));
        }
		...
		function mo_wpns_activate() 
		{
			...
			$this->mo_show_message('malware plugin is already activated, please delete and deactivate it.','ERROR');
			 exit;    
			}
		...
		function mo_wpns_scheduled_scan()
		{
			if(get_option('mo_mmp_scan_initialize')){
				...
				$scan_obj= new Mo_wp_scan_malware();
		...
		{
			global $Mo_wpUtility;
			if( !$Mo_wpUtility->check_empty_or_null( get_option('mo_wpns_registration_status') ) ) {
		...
		function mo_wpns_settings_style($hook)
		{
			if(strpos($hook, 'page_mo_mmp')){
				wp_enqueue_style( 'mo_wpns_admin_settings_style'			, plugins_url('includes/css/style_settings.css', __FILE__),[],MO_WPNS_VERSION);
				wp_enqueue_style( 'mo_wpns_admin_settings_phone_style'		, plugins_url('includes/css/phone.css', __FILE__),[],MO_WPNS_VERSION);
				wp_enqueue_style( 'mo_wpns_admin_settings_datatable_style'	, plugins_url('includes/css/jquery.dataTables.min.css', __FILE__),[],MO_WPNS_VERSION);
				wp_enqueue_style( 'mo_wpns_button_settings_style'			, plugins_url('includes/css/button_styles.css',__FILE__),[],MO_WPNS_VERSION);
                wp_enqueue_style( 'mo_wpns_other_plugins'			, plugins_url('includes/css/other_plugins.css',__FILE__),[],MO_WPNS_VERSION);
		...
		{
			echo Mo_wp_Constants::FOOTER_LINK;
		}
