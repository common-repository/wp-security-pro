 ...
 if(current_user_can( 'manage_options' ) && isset($_POST['option']))
		{
	        switch(sanitize_text_field($_POST['option']))
			{
				case "mo2f_enable_cron_backup":
					mmp_handle_db_enable_form($_POST);				        
                    break;
				case "mo2f_cron_backup_configuration":
					mmp_handle_db_configuration_form($_POST);			    
                    break;
				case "mo2f_enable_cron_file_backup":
					mmp_handle_file_backup_enable_form($_POST);			
                    break;
			}
		} 
...
$enable  =  isset($postData['mo2f_enable_cron_backup_timely']) ? $postData['mo2f_enable_cron_backup_timely'] : '0';
		...
		if(get_option('mo2f_enable_cron_backup') == '0'){
            ...
            do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('CRON_DB_BACKUP_DISABLE'),'ERROR');
            }else{
             do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('CRON_DB_BACKUP_ENABLE'),'SUCCESS');
            }
    }
    ...
    function mmp_handle_db_configuration_form($postData){
        ...
        if($mo2f_cron_hours < 3600){
            do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('INVALID_HOURS'),'ERROR');
        }else{
             ...
             update_option( 'mo2f_enable_cron_file_backup', $enable );
                if(get_option('mo2f_enable_cron_file_backup') == '0'){
                	$handler_obj = new Mo_wp_site_backup;
                 	...
                 	do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('CRON_FILE_BACKUP_DISABLE'),'ERROR');
                 }
                else{
                 	do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('CRON_FILE_BACKUP_ENABLE'),'SUCCESS');
                 }
    }
