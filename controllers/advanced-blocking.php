<?php
	...
	$country		= Mo_wp_Constants::$country;
	...
	function mmp_handle_range_blocking($postedValue)
	{
			...
			if(isset($postedValue['range_'.$i]) && !empty($postedValue['range_'.$i]))
			{
				...
				if(sizeof($range_array) == 2){
					...
					if(filter_var($lowerIP, FILTER_VALIDATE_IP) && filter_var($higherIP, FILTER_VALIDATE_IP)){
						...
						$flag=1;
						do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('INVALID_IP'),'ERROR');
						break;
					}
				}else{
				//error message of invalid format
					$flag=1;
					do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('INVALID_IP_FORMAT'),'ERROR');
					break;
				}
			}
		}
		...
		if($flag == 0){
			do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('IP_PERMANENTLY_BLOCKED'),'SUCCESS');
		}		
	}
	...
	function mmp_handle_user_agent_blocking($postvalue)
	{
		...
		if($user_agent)
			do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('USER_AGENT_BLOCK_ENABLED'),'SUCCESS');
		else
			do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('USER_AGENT_BLOCK_DISABLED'),'ERROR');
	}
