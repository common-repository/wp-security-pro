<?php
	...
	function mmp_handle_support_form($email,$query,$phone)
	{
		...
		{
			do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('SUPPORT_FORM_VALUES'),'SUCCESS');
			return;
		}
		...
		$contact_us = new Mo_wp_MocURL();
		$submited = json_decode($contact_us->submit_contact_us($email, $phone, $query),true);

		if(json_last_error() == JSON_ERROR_NONE && $submited) 
		{
			do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('SUPPORT_FORM_SENT'),'SUCCESS');
			return;
		}
			
		do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('SUPPORT_FORM_ERROR'),'ERROR');
	}