...
if(current_user_can( 'manage_options' )  && isset($_POST['option']))
{
	switch(sanitize_text_field($_POST['option']))
	{
		case "mo_wpns_content_protection":
			mmp_handle_content_protection($_POST);						
			break;
		case "mo_wpns_enable_comment_spam_blocking":
			mmp_handle_comment_spam_blocking($_POST);					
			break;
		case "mo_wpns_enable_comment_recaptcha":
			mmp_handle_comment_recaptcha($_POST);						
			break;
		case "mo_wpns_comment_recaptcha_settings":
			mmp_save_comment_recaptcha($_POST);						
			break;		
	}
}
...
function mmp_handle_content_protection()
{
	...
	$mo_wpns_htaccess_handler = new Mo_wpHandler();
	$mo_wpns_htaccess_handler->update_htaccess_configuration();
	do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('CONTENT_PROTECTION_ENABLED'),'SUCCESS');
}
...
function mmp_handle_comment_spam_blocking($postvalue)
{
	...
	if($enable)
		do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('CONTENT_SPAM_BLOCKING'),'SUCCESS');
	else
		do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('CONTENT_SPAM_BLOCKING_DISABLED'),'ERROR');
}
...
function mmp_handle_comment_recaptcha($postvalue)
{
	...
	if($enable)
		do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('CONTENT_RECAPTCHA'),'SUCCESS');
	else
		do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('CONTENT_RECAPTCHA_DISABLED'),'ERROR');
}

function mmp_save_comment_recaptcha($postvalue){
	update_option('mo_wpns_recaptcha_site_key', sanitize_text_field($postvalue['mo_wpns_recaptcha_site_key']));
	update_option('mo_wpns_recaptcha_secret_key', sanitize_text_field($postvalue['mo_wpns_recaptcha_secret_key']));
