<?php
	...
	$template2	= "Hello ##username##,<br><br>Your account was logged in from new IP Address <b>##ipaddress##</b> on website <b>".get_bloginfo()."</b>. Please <a href='mailto:".Mo_wp_Constants::SUPPORT_EMAIL."'>contact us</a> if you don't recognise this activity.<br><br>Thanks,<br>".get_bloginfo();
        ...
        function mmp_handle_admin_email($postValue)
	{
        ...
        if(mmp_validate_email($_POST['admin_email_address'])){
			...
			do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('EMAIL_SAVED'),'SUCCESS');
	    }else{
	    	do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('INVALID_EMAIL'),'ERROR');
	    }
	}
	...
	function mmp_handle_notify_admin_on_ip_block($postValue)
	{
		...
		if($enable_ip_blocked_email_to_admin)
			do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('NOTIFY_ON_IP_BLOCKED'),'SUCCESS');
		else
			do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('DONOT_NOTIFY_ON_IP_BLOCKED'),'ERROR');
	}
	...
	function mmp_handle_notify_unusual_activity($postValue)
	{
		...
		if($enable_unusual_activity_email_to_user)
			do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('NOTIFY_ON_UNUSUAL_ACTIVITY'),'SUCCESS');
		else
			do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('DONOT_NOTIFY_ON_UNUSUAL_ACTIVITY'),'ERROR');
	}
	...
	function mmp_handle_custom_template($template1,$template2=null)
	{
		...
		do_action('mo_mmp_show_message',Mo_wp_Messages::showMessage('TEMPLATE_SAVED'),'SUCCESS');
	}