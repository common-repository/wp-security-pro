<?php
	class Mo_wp_reCaptcha
	{
		...
		function handle_recaptcha()
		{
			...
			{ 
				$current_url = home_url(sanitize_text_field($_SERVER['REQUEST_URI']));
				...
				{	
						if(array_key_exists('g-recaptcha-response',$_POST))
						{
							...
							$Mo_wp_MocURL 	= new Mo_wp_MocURL;
							$response 	= $Mo_wp_MocURL->validate_recaptcha($userIp,sanitize_text_field($_POST['g-recaptcha-response']));
						...
						Mo_wp_show_google_recaptcha_form();
				}
			}
		}
		...
		function custom_login_fields()
		{
			if(get_option('mo_wpns_activate_recaptcha_for_login'))
			{
				...
				echo "<script src='".Mo_wp_Constants::RECAPTCHA_URL."'></script>";
		...
		public static function recaptcha_verify($response)
		{
			...
			$userIp 	= $Mo_wpUtility->get_client_ip();
			$Mo_wp_MocURL 	= new Mo_wp_MocURL;
			$response 	= $Mo_wp_MocURL->validate_recaptcha($userIp,$response);
			$content	= json_decode($response, true);
