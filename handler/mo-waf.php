<?php
	...
	if(mysqli_select_db($dbcon,$dbD['DB_NAME']))
	{
        ...
        if(isset($row['option_value']) && $row['option_value']=='HtaccessLevel'){
            ...
            $query =  'select * from '.$prefix.'wpns_blocked_ips where ip_address="'.$ipaddress.'";';
            $results = mysqli_query($dbcon,$query);
            if($results)
            {
                $row = mysqli_fetch_array($results);
                $query = 'select * from '.$prefix.'wpns_whitelisted_ips where ip_address="'.$ipaddress.'";';
                $results = mysqli_query($dbcon,$query);
                if($results)
                {
                        ...
                        else
                        {
                            ...
                            if($current_time>$results['time']+60)
                            {
                                ...
                                if($results)
                                {
                                    ...
                                    {
                                        $query ="insert into ".$prefix."wpns_blocked_ips values(NULL,'".$ipaddress."','Rate limit exceed',NULL,".time().");";
                                        $results = mysqli_query($dbcon,$query);
                                    }
                            header('HTTP/1.1 403 Forbidden');
                            include_once("mo-error.php");
                                                ...
                                                if($annomalyS>=5 || $SQLScore>=10 || $XSSScore >=10)
                                                {
                                                    ...
                                                    if($rows['count(*)']>$limitAttack)
                                                    {
                                                        ...
                                                        if($results)
                                                        {
                                                            ...
                                                            {
                                                                $query ="insert into ".$prefix."wpns_blocked_ips values(NULL,'".$ipaddress."','attack limit exceed',NULL,".time().");";
                                                                $results = mysqli_query($dbcon,$query);
                                                            }
                                                        }
                                                    }
