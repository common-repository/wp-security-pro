...
echo 					"</td><td>".esc_attr(date("M j, Y, g:i:s a",$blockedip->created_timestamp))."</td><td><a onclick=unblockip('".esc_attr($blockedip->id)."')>Unblock IP</a></td></tr>";
					} 
		...
		<div class="mo_wpns_setting_layout">
			...
			<table id="whitelistedips_table" class="display">
				...
				<tbody>';
					...
					{
						echo "<tr><td>".esc_attr($whitelisted_ip->ip_address)."</td><td>".esc_attr(date("M j, Y, g:i:s a",$whitelisted_ip->created_timestamp))."</td><td><a onclick=removefromwhitelist('".esc_attr($whitelisted_ip->id)."')>Remove</a></td></tr>";
					} 
