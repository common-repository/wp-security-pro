...
class Mo_wpUtility
{
	...
	public static function check_if_valid_email($email)
	{
		...
		if(sizeof($emailarray)==2)
			return in_array(trim($emailarray[1]), Mo_wp_Constants::$domains);
		else
			return false;
	}
	...
	function verify_recaptcha($response)
	{
		...
		if(!empty($response))
		{
			if(!Mo_wp_reCaptcha::recaptcha_verify($response))
	...
	function getMessageContent($reason,$ipAddress,$username=null,$fromEmail=null)
	{
		switch($reason)
		{
			case Mo_wp_Constants::LOGIN_ATTEMPTS_EXCEEDED:
				...
				return $content;
			case Mo_wp_Constants::IP_RANGE_BLOCKING:
				...
				return $content;
			case Mo_wp_Constants::LOGGED_IN_FROM_NEW_IP:
				...
				return $content;
			case Mo_wp_Constants::FAILED_LOGIN_ATTEMPTS_FROM_NEW_IP:
