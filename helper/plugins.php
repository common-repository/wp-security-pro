<?php
	...
	class Mo_wp_FeaturePluginInterface
	{
	...
	class Mo_wp_TwoFAPlugin extends Mo_wp_FeaturePluginInterface
	{	
		...
		function getstatus()
		{
			...
			if(isset($all_plugins[Mo_wp_Constants::TWO_FACTOR_SETTINGS]))
				$status = is_plugin_active(Mo_wp_Constants::TWO_FACTOR_SETTINGS) ? 'ACTIVE' : 'INSTALLED';
			return $status;
		}
		...
		function updatePluginConfiguration()
		{
			...
			if(!get_option( 'mo2f_customerKey') || !get_option( 'mo2f_api_key') || !get_option( 'mo2f_customer_token') || !get_option( 'mo2f_app_secret'))
			{
				...
				update_option( 'mo2f_host_name' 		,Mo_wp_Constants::HOST_NAME);
	...
	class Mo_wp_OTPPlugin extends Mo_wp_FeaturePluginInterface
	{
		function getstatus()
		{
		    $all_plugins = get_plugins();
			$status = 'NOT_INSTALLED';
			if(isset($all_plugins[Mo_wp_Constants::OTP_VERIFICATION_SETTINGS]))
				$status = is_plugin_active(Mo_wp_Constants::OTP_VERIFICATION_SETTINGS) ? 'ACTIVE' : 'INSTALLED';
			return $status;
		}
	...
	class Mo_wp_SocialPlugin extends Mo_wp_FeaturePluginInterface
	{
		function getstatus()
		{
		    $all_plugins = get_plugins();
			$status = 'NOT_INSTALLED';
			if(isset($all_plugins[Mo_wp_Constants::SOCIAL_LOGIN_SETTINGS]))
				$status = is_plugin_active(Mo_wp_Constants::SOCIAL_LOGIN_SETTINGS) ? 'ACTIVE' : 'INSTALLED';
			return $status;
		}
	}